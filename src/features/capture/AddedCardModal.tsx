...
/**
 * The "Added" confirmation bottom sheet. One row per added transaction (tap a row
 * to edit/discard it), an ✕ to dismiss (keeps the entries), and Undo (deletes
 * them all). Editing a row swaps the edit form into THIS SAME modal rather than
 * stacking a second one — stacked modals + a native date picker deadlock on iOS.
 */
export function AddedCardModal({
  ...
  saved: Transaction[];
  locale: Locale;
  /** When set, the modal shows the edit form for this row instead of the card. */
  ...
  return (
    <Modal
      ...
      transparent
      animationType="slide"
      onRequestClose={editing ? onEditCancel : onDismiss}
    >
      <Pressable
        style={{ flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.6)' }}
        onPress={editing ? onEditCancel : onDismiss}
      >
        {editing ? (
          // ── Edit a tapped row (same modal — no stacking) ──────────────────
